...
describe('"Search with filters" component', function() {
    ...
    describe ('filter events', function () {
        ...
        xit ('should repopulate the results on submit of the filters', function () {
        ...
        xit ('should append a new page of results on click of "load more"', function () {
        ...
        xit ('should repopulate the results with the relevant page on click of a pagination link', function () {
...
////
    xdescribe('date filter', function () {
        let dateFromInputGroup,
            dateFromInput,
            dateToInputGroup,
            dateToInput;
        ...
        xit('date picker: should set dates in associated text inputs', function () {
            // get datepicker
            const datePicker = $('#date-from').closest('.date-entry').find('.pika-single');

            // click on an option
            const dateToPick = datePicker.find('td:not(.is-disabled) .pika-day:last');

            const expectedDate = dates.leadingZeroes(dateToPick.data('pika-day'), 2) + '/' + dates.leadingZeroes(dateToPick.data('pika-month') + 1, 2)  + '/' + dateToPick.data('pika-year').toString();
        ...
        it ('date picker: there should be a way of opening the date picker calendar', function () {
            ...
            const showButton = $('#date-from').closest('.date-entry').find('.js-show-calendar');
        ...
        it ('date picker: there should be a way of closing the date picker calendar', function () {
            ...
            const showButton = $('.js-show-calendar:first');
            showButton.trigger('click');

            const closeButton = showButton.closest('.date-entry').find('.js-close-calendar');
        ...
        it ('date picker: only one date picker calendar should be visible at a time', function () {
            ...
            const showButton1 = $('#date-from').closest('.date-entry').find('.js-show-calendar');
            showButton1.trigger('click');

            expect($('.date-entry__calendar--open').length).toEqual(1);

            const showButton2 = $('#date-to').closest('.date-entry').find('.js-show-calendar');
            showButton2.trigger('click');

            expect($('.date-entry__calendar--open').length).toEqual(1);
        });

        xdescribe('date validation', function () {
            it ('should fail validation if an input date is an incorrect format', function () {
                spyOn(search.searchUtils, 'addError');

                const input = $('#date-from');
                input.val('bananas');

                const isValid = search.validateDateInput(input);
            ...
            it ('should fail validation if the "from" date is later than the "to" date', function () {
                ...
                const isValid = search.validateDateInput(inputFrom);
