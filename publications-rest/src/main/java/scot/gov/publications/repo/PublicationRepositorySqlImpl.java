...
public class PublicationRepositorySqlImpl implements PublicationRepository {
    ...
    /**
     * Allowlist of the fields a caller may sort by, mapped to their column name. Sort input is
     * never concatenated into the query directly to avoid SQL injection via the sort param.
     */
    private static final java.util.Map<String, String> SORTABLE_FIELDS = new java.util.LinkedHashMap<>();
    static {
        SORTABLE_FIELDS.put("title", "title");
        SORTABLE_FIELDS.put("isbn", "isbn");
        SORTABLE_FIELDS.put("filename", "filename");
        SORTABLE_FIELDS.put("createddate", "createddate");
        SORTABLE_FIELDS.put("embargodate", "embargodate");
        SORTABLE_FIELDS.put("state", "state");
        SORTABLE_FIELDS.put("username", "username");
        SORTABLE_FIELDS.put("lastmodifieddate", "lastmodifieddate");
    }

    private static final String DEFAULT_SORT = "lastmodifieddate";
    ...
    public ListResult list(int page, int size, String title, String isbn, String filename, String username, String sort, String dir)
        ...
        try {
            ...
            if (StringUtils.isNotBlank(username)) {
                whereClause.append("LOWER(username) LIKE ? AND ");
                whereArgs.add(like(username));
            }
            ...
            sql = sql.replaceAll("<ORDERBY>", orderByClause(sort, dir));
    ...
    private String orderByClause(String sort, String dir) {
        String column = SORTABLE_FIELDS.getOrDefault(sort, SORTABLE_FIELDS.get(DEFAULT_SORT));
