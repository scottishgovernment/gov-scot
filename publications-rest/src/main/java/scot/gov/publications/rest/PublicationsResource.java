...
public class PublicationsResource {
    ...
    public Response list(
            ...
            @DefaultValue("") @QueryParam("dir") String dir) {

        LOG.info("LIST publications page={} size={} title={} isbn={} filename={} username={} sort={} dir={}",
                page, size, title, isbn, filename, username, sort, dir);
        try {
            if (!isAuthorized(UserSession.get().getJcrSession())) {
                return Response.status(Response.Status.FORBIDDEN).build();
            }
            ListResult result = repository.list(page, size, title, isbn, filename, username, sort, dir);
