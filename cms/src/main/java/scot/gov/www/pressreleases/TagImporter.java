...
import scot.gov.www.pressreleases.prgloo.Calls;
import scot.gov.www.pressreleases.prgloo.PRGloo;
import scot.gov.www.pressreleases.prgloo.PRGlooFactory;
...
public class TagImporter {
    ...
    TagGroups fetchTags() {
        ...
        Call<TagGroups> call = prgloo.tagGroups();
        Response<TagGroups> response = Calls.call(call);
        if (!response.isSuccessful()) {
            throw new PressReleaseImporterException("Could not get tags: " + response);
        }
        TagGroups groups = response.body();
        LOG.info("Found {} tag groups", groups.getTagGroupList().size());
        return groups;
    }
