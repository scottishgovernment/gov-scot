...
public class PressReleaseConverter {
    ...
    public PressRelease convert(PressReleaseItem from) {
        ...
        if (from.getContentStreamId().equals(PRGlooFactory.PRESS_RELEASE_STREAM_ID)) {
            convertContact(to, from.getSender());
        }

        if (from.getContentStreamId().equals(PRGlooFactory.SPEECH_STREAM_ID)) {
            to.setPublicationType(SPEECH_NODE);
        }

        if (from.getContentStreamId().equals(PRGlooFactory.CORRESPONDENCE_STREAM_ID)) {
