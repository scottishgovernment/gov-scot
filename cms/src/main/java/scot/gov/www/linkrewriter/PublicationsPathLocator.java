 ...
 * </ul>
 */
class PublicationsPathLocator implements PathLocatorStrategy {
    ...
    private Node resolvePublicationSubPath(Session session, Node indexHandle, String subPath,
        ...
        if (subPath.equals(DOCUMENTS_SUBPATH) || subPath.startsWith(DOCUMENTS_SUBPATH + "/")) {
            if (subPath.startsWith(DOCUMENTS_SUBPATH + "/")) {
                ...
                LOG.info("DOCUMENT-FILE-LINK PublicationsPathLocator: '{}' links to document file '{}', rewriting to first document in publication",
            ...
            Node first = firstDocumentHandle(session.getNode(docsPath));
            if (first == null) {
