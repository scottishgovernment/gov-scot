 ...
 *       or the {@code index} child of a folder.</li>
 ...
 * </ol>
 */
class DirectPathLocator implements PathLocatorStrategy {
    ...
    private Optional<Node> findByJcrPath(Session session, String path) throws RepositoryException {
        ...
        // Publication documents folder: return the first document handle
        if (node.isNodeType("hippostd:folder") && DOCUMENTS_FOLDER_NAME.equals(node.getName())) {
            return firstDocumentHandle(node, jcrPath);
        }
    ...
    private Optional<Node> firstDocumentHandle(Node documentsFolder, String jcrPath)
            throws RepositoryException {
        javax.jcr.NodeIterator it = documentsFolder.getNodes();
        while (it.hasNext()) {
        ...
        LOG.warn("DirectPathLocator: documents folder at {} contains no handles", jcrPath);
        return Optional.empty();
    }
