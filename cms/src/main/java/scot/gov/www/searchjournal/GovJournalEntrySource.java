...
import scot.gov.publishing.journal.JournalEntrySource;
...
public class GovJournalEntrySource implements JournalEntrySource {
    ...
    private static final String CONTENTS_PAGE_PROPERTY = "govscot:contentsPage";
    ...
    @Override
    public List<JournalEntry> entriesForEvent(HippoWorkflowEvent event) throws RepositoryException {
        if (!event.success()) {
    ...
    List<JournalEntry> entriesForMoveEvent(HippoWorkflowEvent event) throws RepositoryException {
    ...
    List<JournalEntry> entriesForMovedDocumentsFolder(HippoWorkflowEvent event) throws RepositoryException {
    ...
    List<JournalEntry> entriesForMovedPagesFolder(HippoWorkflowEvent event) throws RepositoryException {
    ...
    List<JournalEntry> entriesForHandle(Node handle, String action) throws RepositoryException {
    ...
    List<JournalEntry> journalEntriesForPublication(Node variant, Node handle, String action) throws RepositoryException {
        Node publication = getPublication(variant);
        String publicationHandleId = publication.getParent().getIdentifier();
        JournalEntry entry = newEntry(publicationUrl(publication, variant, action, handle.getPath()), action, publicationHandleId);
        ...
        if (isAnyNodeType(variant, "govscot:PublicationPage")) {
            JournalEntry documentsEntry = documentsEntry(needsDocumentsPage, publication, action);
            JournalEntry publicationEntry = publicationEntry(publication, action);
    ...
    List<JournalEntry> addPageEntries(boolean hasPages, Node publicationFolder, Node publication, String action) throws RepositoryException {
        List<JournalEntry> journalEntries = new ArrayList<>();
        if (hasPages) {
    ...
    JournalEntry documentsEntry(boolean needsDocumentsPage, Node publication, String action) throws RepositoryException {
    ...
    JournalEntry publicationEntry(Node publication, String action) throws RepositoryException {
    ...
    JournalEntry publicationPageEntry(Node page, Node publication, String action) throws RepositoryException {
        String entryAction = actionDependingOnState(publication, action);
        return newEntry(publicationPageUrl(publication, page, entryAction), entryAction, publication.getParent().getIdentifier());
    }

    JournalEntry publicationChapterEntry(Node chapter, Node publication, String action) throws RepositoryException {
        String entryAction = actionDependingOnState(publication, action);
    ...
    JournalEntry depublishSubfolderEntry(String oldPath, String pageTitle, boolean isPages) {
    ...
    JournalEntry depublishPublicationEntryForSubfolderMove(String oldPath) {
        String[] pathSections = oldPath.split("/");
    ...
    boolean isPublicationContentsPage(Node variant) throws RepositoryException {
        return variant.hasProperty(CONTENTS_PAGE_PROPERTY)
                && variant.getProperty(CONTENTS_PAGE_PROPERTY).getBoolean();
    }
    ...
    private JournalEntry newEntry(String url, String action) {
        JournalEntry entry = new JournalEntry();
        entry.setUrl(url);
        entry.setAction(JournalAction.fromBloomreachAction(action));
        return entry;
    }

    private JournalEntry newEntry(String url, String action, String contentId) {
        JournalEntry entry = newEntry(url, action);
