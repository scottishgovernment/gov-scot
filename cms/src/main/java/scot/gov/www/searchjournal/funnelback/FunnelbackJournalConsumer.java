...
/**
 * {@link JournalConsumer} that pushes content to FunnelbackIndexer's push API.
 *
 * <p>HTML is fetched via the injected {@link SiteContentFetcher}. The FunnelbackIndexer collection
 * is determined at consume time by inspecting the document's JCR node type, so that
 * collection assignment (a FunnelbackIndexer-specific detail) is not mixed into the journal
 * recording layer.
 */
public class FunnelbackJournalConsumer implements JournalConsumer {
    ...
    public FunnelbackJournalConsumer(FunnelbackIndexer funnelback, SiteContentFetcher fetcher, Session session) {
    ...
    @Override
    public void consume(JournalEntry entry) throws JournalConsumerException {
        switch (entry.getAction()) {
            ...
            case UNPUBLISH:
                ...
                break;
            default:
                LOG.warn("Unrecognised action '{}' for {}, skipping", entry.getAction(), entry.getUrl());
        }
    }
    ...
    private void depublish(JournalEntry entry) throws JournalConsumerException {
        try {
            String collection = collectionFor(entry);
            if (collection == null) {
            ...
            funnelback.unpublish(collection, entry.getUrl());
        } catch (FunnelbackException e) {
    ...
    /**
     * Determines the FunnelbackIndexer collection for the given journal entry.
     *
     ...
     * entry to be skipped.
     */
    String collectionFor(JournalEntry entry) throws RepositoryException {
    ...
    /**
     * Resolves the FunnelbackIndexer collection for a child node (page, document, or chapter) by
    ...
    /**
     * Returns the FunnelbackIndexer collection name for a publication variant node by reading
