...
public class LinkRewriteJob implements RepositoryJob {
    ...
    void doExecute(Session session, FeatureFlag flag) throws RepositoryException {
        ...
        try {
            ...
            saver.forceSave();
            logStats(stats);
            LOG.info("LinkRewriteJob: stopped early by operator", e);
            return;
    ...
    private LinkReplacement processLink(Node htmlNode, String href, Stats stats) throws RepositoryException {
        ...
        return new LinkReplacement(href, target, strategy.lastResolutionType);
    }
    ...
    private static void incrementTypeCounter(GovScotLinkRewriteStrategy.ResolutionType type, Stats stats) {
        switch (type) {
            case NEWS:
                stats.rewrittenByNews++;
                break;
            case PUBLICATIONS:
                stats.rewrittenByPublications++;
                break;
            case ISBN:
                stats.rewrittenByIsbn++;
                break;
            case PATH:
                stats.rewrittenByPath++;
                break;
            case TOPIC:
                stats.rewrittenByTopic++;
                break;
            case REDIRECT:
                stats.rewrittenByRedirect++;
                break;
        }
    }
