...
public class SsoLoginPlugin extends CustomLoginPlugin {
    ...
    class SsoLoginForm extends DefaultLoginPlugin.TimeZonePanel {
        ...
        public SsoLoginForm(String id, LoginConfig config, LoginHandler handler) {
            ...
            Label ssoLoginButton = new Label("sso-login", new ResourceModel("sso.login"));
            boolean ssoEnabled = SsoConfig.get().mode() != SsoConfig.Mode.OFF;
            ssoLoginButton.setVisible(ssoEnabled);
            form.addLabelledComponent(ssoLoginButton);
        }
