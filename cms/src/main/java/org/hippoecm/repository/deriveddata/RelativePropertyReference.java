...
public class RelativePropertyReference extends PropertyReference {
    ...
    Value[] getPropertyValues(Node modified, Collection<String> dependencies) throws RepositoryException {
        // Note: special case for 'path'
        //       this has been added by govscot in order to be able to access the jcr path from derived data functions.
        //       it should be removed when axcess to path has been added to bloomreach, see
        //       https://issues.onehippo.com/browse/CMS-16025
        String relativePath = getRelativePath();
        if ("path".equals(relativePath)) {
            return new Value[] { new StringValue(modified.getPath()) };
        }

        final Property property = JcrUtils.getPropertyIfExists(modified, relativePath);
        if (property != null) {
