...
public class TopicLinkProcessor extends HstLinkProcessorTemplate {

    private static final Logger LOG = LoggerFactory.getLogger(TopicLinkProcessor.class);
    ...
    private boolean isSiteItemFullLink(HstLink link) {
        return link.getPath().startsWith("topics/") && link.getPathElements().length == 2;
    }
    ...
    protected HstLink doPreProcess(HstLink link) {
        ...
        try {
            ...
            String newPath = String.format("topics/%s", StringUtils.substringAfter(handle.getPath(), "topics/"));
